...
int main() {
	...
	Смогу чистить его память только если создам переменную, которая равна максимальному из всех
	id на данный момент. Но т.к. нужен именно пул id-шников, в любом случае потребуется вектор, который
	будет постоянно заполняться*/
	...
	while (video[0].read(frame[0]) && video[1].read(frame[1]))
	{
		...
		for (auto& tracker : trackers) 
		{
			...
			P.s. жертвую читаемостью во избежание копипысты, которая была в прошлый раз*/
